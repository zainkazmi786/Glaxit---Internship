...
// Create new lesson (Instructor only for their own courses)
...
// Update lesson (Instructor only for their own courses)
router.patch('/:id', authenticate, authorize('instructor'), async (req, res) => {
  try {
    const lesson = await Lesson.findById(req.params.id).populate('course');
...
// Delete lesson (Instructor only for their own courses)
